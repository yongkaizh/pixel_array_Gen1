...
export function generateCentering(builder: SkillBuilder, config: LayoutConfig): void {
  ...
  builder.push("        )");
  builder.push("        d_width = urx - llx");
  builder.push("        h_height = ury - lly");
  builder.push("        P_ll = list(llx lly)");
  builder.push("        ");
  builder.push("        ; Transform lower-left of the shape to the first instance of the mosaic (x0, y0)");
  builder.push("        P_first_ll = dbTransformPoint(P_ll list(car(maxActiveInst~>xy) cadr(maxActiveInst~>xy)) maxActiveInst~>orient)");
  builder.push("        x0 = car(P_first_ll)");
  builder.push("        y0 = cadr(P_first_ll)");
  builder.push("        ");
  ...
  builder.push("        ");
  builder.push("        ; Formula: Center_X = x0 + (d + (n - 1) * pitch_x) / 2.0");
  builder.push("        ; Formula: Center_Y = y0 + (h + (m - 1) * pitch_y) / 2.0");
  builder.push("        cx = x0 + (d_width + (u_cols - 1) * u_dx) / 2.0");
  builder.push("        cy = y0 + (h_height + (u_rows - 1) * u_dy) / 2.0");
